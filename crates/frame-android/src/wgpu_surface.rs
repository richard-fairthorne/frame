...
impl AndroidSurfaceProvider {
    ...
    pub fn init_surface_from_native_window(
        &mut self,
        window_id: WindowId,
        ...
        let raw_handle = native_window.raw_window_handle();
        ...
        let dev_id = block_on(self.render_ctx.device(Some(&surface))).ok_or(
            SurfaceError::SurfaceCreationFailed("no compatible device".into()),
        )?;
...
impl Default for AndroidSurfaceProvider {
    fn default() -> Self {
        Self::new()
    }
}
